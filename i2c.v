    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            case (state)
                ...
                ADDR: begin
                    sda_en <= 1;
                    // إرسال العنوان ثم بت القراءة/الكتابة
                ...
                WRITE: begin // حالة اختيارية لو احتاجتي ترسلي أوامر للحساس
                    sda_en <= 1;
                    ...
                    if (clk_cnt == 99) begin
                        clk_cnt <= 0;
                        if (bit_cnt > 0) bit_cnt <= bit_cnt - 1;
                        else state <= STOP; // أو الانتقال لـ ACK2
                    end
                end
                ...
                READ: begin
                    ...
                    clk_cnt <= clk_cnt + 1;
                    if (clk_cnt == 25) i2c_scl <= 1;
                    // لقط البيانات في منتصف النبضة (Sampling)
                ...
                NACK: begin // الماستر يرسل NACK بعد قراءة البايت
                    sda_en <= 1; sda_out <= 1; 
                    clk_cnt <= clk_cnt + 1;
                    if (clk_cnt == 25) i2c_scl <= 1;
                    if (clk_cnt == 75) i2c_scl <= 0;
